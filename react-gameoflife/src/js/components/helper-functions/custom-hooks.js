...
import getNeighbors from "./grid-neighbours";
...
export const useGrid = () => {
  ...
  const stepThroughAutomata = () => {
    let validGrid = false; //  Set a variable to determine if the grid is valid. (we can make a mutation based on the rules)
    ...
    const nextGeneration = grid.map((cell, i) => {
      let neighbors = getNeighbors(i, gridSize, gridSize); //Map over the current grid, saving this new map as nextGeneration and check for the current cell’s neighbors using the getNeighbors helper function.
      ...
      //Initialize livingNeighbors to 0, then check all of the neighbors of the current cell to see if any of
      //them are alive.For each neighbor that is alive, increment livingNeighbors by 1.
      ...
      return cell;
      // Based on the number of living neighbors of the current cell, check the rules of the game,and toggle
      // the current cell to either be alive if it was dead, die if it was alive, or remain the same. Also set validGrid to
      // be true if we make a mutation, because if we’re able to make a mutation it means that the grid is valid.
    });

    // If the grid is valid, increment the current generation; else let the user know that the grid is invalid. Finally,
    // set the grid to be the nextGeneration. (nextGeneration will be the old grid, but with the new changes made based
    // on the rules)
    if (validGrid) {
